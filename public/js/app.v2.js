 ...
 * - Scenario picker: populates from /public/data/v2-index.json and reboots on change
...
(() => {
  ...
  async function fetchJSON(url, opts = {}) {
    ...
    if (CONFIG.noStore) {
      ...
      try {
        ...
        url = u.toString();
      } catch (e) { /* ignore URL errors for relative paths */ }
    }
  ...
  async function resolveScenarioPathById(id) {
    try {
      ...
      const entries = Array.isArray(idx) ? idx : (idx && idx.entries) ? idx.entries : [];
  ...
  function getScenarioPicker() {
    return document.getElementById("scenarioPicker");
  }

  async function populateScenarioPicker(currentId) {
    const sel = getScenarioPicker();
    if (!sel) return;
    ...
    try {
      ...
      const items = entries
        .map(x => ({ id: x.id || x.slug, title: x.title || x.name || x.id || x.slug }))
        .filter(x => x.id)
      ...
      const wanted = currentId || (window.__SCENARIO_ID__) || (items[0] && items[0].id);
      ...
      warn("Failed to populate scenario picker:", e);
      sel.innerHTML = '<option disabled>Index not found</option>';
    }
  }
