// admin-logs.v9.5.4.js — telemetry status badge + tolerant loader + local buffer
(function () {
  "use strict";
  ...
  var TELEMETRY_GRACE_MS = 15000; // 15s window for "active" status
  ...
  function setTelemetryStatus(ok, detail) {
    ...
    b.textContent = ok ? "Telemetry: active" : "Telemetry: waiting…";
    b.style.borderColor = ok ? "rgba(34,197,94,.6)" : "rgba(245,158,11,.6)"; // green or amber
  ...
  function updateTelemetryStatusTimer() {
    ...
    const tick = function () {
      ...
      setTelemetryStatus(ok, ok ? ("Last event " + Math.floor((now - lastEventTs)/1000) + "s ago") : "No events recently");
    };
    tick();
  ...
  function bind() {
    ...
    if (sendTest) {
      sendTest.addEventListener("click", function () {
        ...
        if (state.useLocal) loadLocal(); else lastEventTs = Date.now();
      });
    }
  ...
  async function init() {
    ...
    // initial load + telemetry status
    if (state.useLocal) loadLocal(); else await loadEndpoint();
    populateTypes();
    // If there are zero events at start, show waiting immediately
