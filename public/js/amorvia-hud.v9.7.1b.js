...
// Amorvia HUD v9.7.1b – overwrite (JS)
// - Only mounts into existing #hud-toolbar / .hud-toolbar
// - Adds delayed mount via MutationObserver (handles late-rendered toolbars)
// - Removes any stray duplicate sticky bars created by previous versions
...
(function () {
  ...
  function cleanupStrayBars(hostToolbar) {
    const bars = Array.from(document.querySelectorAll("body > .amorvia-toolbar"));
    bars.forEach(el => {
  ...
  function attemptMountWithObserver(timeoutMs=5000) {
    const existing = q("#hud-toolbar") || q(".hud-toolbar");
    if (existing) return mountIntoToolbar(existing);

    // Observe for a bit in case the app renders toolbar later
    const observer = new MutationObserver(() => {
      const t = q("#hud-toolbar") || q(".hud-toolbar");
      if (t) {
        observer.disconnect();
        mountIntoToolbar(t);
      }
