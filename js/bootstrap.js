...
async function loadChosenApp() {
  ...
  try {
    if (mode === 'v2') {
      const app = await import('/js/app.v2.js?v=' + (window.AMORVIA_BUILD || Date.now()));
      await Promise.allSettled([
        import('/js/addons/extras-tabs.js?v=' + (window.AMORVIA_BUILD || Date.now())),
        import('/js/addons/art-loader.js?v=' + (window.AMORVIA_BUILD || Date.now())),
      ]);
      app?.init?.();
    } else {
