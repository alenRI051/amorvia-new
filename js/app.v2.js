// /js/app.v2.js (compat + engine autoload)
import { /* keep ESM */ } from 'data:text/javascript,export default null';
const BUILD = (window.AMORVIA_BUILD || 'v0.6-2025-09-01');
...
export async function loadScenarioById(id){
  ...
  if (!graph?.startId || !graph.nodes?.[graph.startId]) {
    try { const m = await import(COMPAT_URL); if (typeof m.toGraph === 'function') graph = m.toGraph(raw); } catch {}
  }
  if (!graph?.startId || !graph.nodes?.[graph.startId]) {
...
export async function init(){
  ...
  try {
    if (pick && !pick.options.length) {
      ...
      for (const s of list) { const o = document.createElement('option'); o.value = s.id; o.textContent = s.title || s.id; pick.appendChild(o); }
    }
