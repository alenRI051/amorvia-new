...
async function getEngine(){
  ...
  for (const p of paths){
    try{
      ...
      if (E && (E.loadScenario || E.start || E.startAct)){ window.ScenarioEngine = E; console.info('[v2] engine from', p); return E; }
    }catch(e){ lastErr = e; }
  }
...
export async function loadScenarioById(id){
  const raw = await fetch(`/data/${id}.v2.json?ts=${Date.now()}`, { cache:'no-store' }).then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); });
...
export async function init(){
  ...
  if (pick && !pick.options.length){
    try{
      ...
      if (pick.options.length) pick.value = pick.options[0].value;
    }catch(e){ console.warn('[v2] index failed', e); }
  }
