...
export function v2ToGraph(v2) {
  if (!v2 || !Array.isArray(v2.acts)) throw new Error("v2ToGraph: invalid v2 payload (missing acts)");
  const nodes = {};
  let startId = null;
  ...
  if (v2.meters && typeof v2.meters === "object") {
    for (const [key, def] of Object.entries(v2.meters)) {
      const val = typeof def === "number" ? def :
  ...
  v2.acts.forEach((act, ai) => {
    ...
    steps.forEach((step, si) => {
      ...
      const nextId =
        (si + 1 < steps.length) ? `a${ai+1}s${si+2}` :
        (ai + 1 < v2.acts.length) ? `a${ai+2}s1` : null;

      if (typeof step === "string") {
        nodes[id] = nextId ? { id, text: step, next: nextId } : { id, text: step, type: "end" };
      } else if (step && typeof step === "object") {
        const text = String(step.text ?? "");
        const choicesSrc = Array.isArray(step.choices) ? step.choices
        ...
        if (choicesSrc && choicesSrc.length) {
          ...
          nodes[id] = nextId ? { id, text, next: nextId } : { id, text, type: "end" };
        }
      } else {
        nodes[id] = nextId ? { id, text:"", next: nextId } : { id, text:"", type:"end" };
      }
  ...
  return { title: v2.title || v2.id || "Scenario", startId: startId || "a1s1", nodes, meters };
}
